...
def main():
    ...
    f = open('/Users/h3ct0r/Desktop/concorde/LINKERN/result', 'w')

    f.write(str(len(best)) + ' ' + str(len(best)) + '\n')
    for i in xrange(len(best)):
        if i >= len(best) - 1:
            f.write(str(best[i]) + ' ' + str(best[0]) + ' 1\n')
        else:
            f.write(str(best[i]) + ' ' + str(best[i + 1]) + ' 1\n')
        pass
